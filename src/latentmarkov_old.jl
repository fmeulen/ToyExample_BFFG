...
function generate_track(θ, X, Πroot)             # Generate exact track + observations
    Λ = Λi(θ)
...
function loglik_and_bif(θ, Πroot, X, Y)
    N = length(Y) - 1
...
negloglik(Πroot, X, Y) = (θ) ->  -loglik_and_bif(θ, Πroot, X, Y).ll
∇negloglik(Πroot, X, Y) = (θ) -> ForwardDiff.gradient(negloglik(Πroot, X, Y), θ)
loglik(Πroot, X, Y) = (θ) -> loglik(θ, Πroot, X, Y) 


function guided_track(θ, Πroot, X, H)# Generate approximate track
    N = length(H) - 1
...
# generate track  
U, Y =  generate_track(θ, X, Πroot) 
# backward filter
ll, H = loglik_and_bif(θ, Πroot, X, Y)
# sample from conditioned process
Uᵒ = guided_track(θ, Πroot, X, H)
...
der_fwdiff = ∇negloglik(Πroot, X, Y)(θ0)
der_findiff = FiniteDiff.finite_difference_gradient(negloglik(Πroot, X, Y), θ0)
...
optimize(negloglik(Πroot, X, Y), θ) 
...
# one chain
mcmc_with_warmup(Random.default_rng(2), ∇P, 1000)
