...
function likelihood(θ, Λ, Πroot, ys)
    N = length(ys) - 1
    K = Ki(θ)
    hprev = Λ[:,ys[N+1]]                   
    for i=N:-1:1
...
negloglik(Λ, Πroot, ys) = (θ) ->  -log(likelihood(θ, Λ, Πroot, ys))
#negloglik(Λ, Πroot, ys) = (θ) ->  -log(likelihood_and_guided(Ki(θ), Λ, Πroot, ys)[1])  # autodiff does not work on this one

∇negloglik(Λ, Πroot, ys) = (θ) -> ForwardDiff.derivative(negloglik(Λ, Πroot, ys), θ)